        ...
        <div className="job-category-page bg-white p-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {jobCategories.map(category => (
                    <div 
                        key={category.id} 
                        className="bg-[#f9f9f9] p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105"
                    >
                        <h2 className="text-xl font-semibold text-[#19995C]">{category.title}</h2>
                        <p className="text-gray-600">{category.jobs} jobs available</p>
                        <a
                            href={`/jobs/${category.title.toLowerCase().replace(/\s+/g, '-')}`}
                            className="text-[#19995C] hover:underline font-medium"
                        >
                            View Jobs
                        </a>
                    </div>
